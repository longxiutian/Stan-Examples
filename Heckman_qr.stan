...
parameters {
  ...
  vector[K] theta_s;      // coefficients on Q_ast
  vector[K] theta_o;      // coefficients on Q_ast
...
model {
  ...
  matrix[N, 2] XB = Q_ast * append_col(theta_s, theta_o);
...
generated quantities {
  ...
  beta_s = R_ast_inverse * theta_s; // coefficients on x
  beta_o = R_ast_inverse * theta_o; // coefficients on x
}
